...
export function SongPreview({ song, idx }) {
    ...
    async function onRemove(station, song) {
        try {
            const songIdx = station.songs.findIndex(currSong => currSong.id === song.id)
    ...
    async function onLike() {
        if (user.stations[0].songs.some(currSong => currSong.id === song.id)) {
            ...
            const songIdx = user.stations[0].songs.findIndex(currSong => currSong.id === song.id)
        ...
        <>
            <div className="flex justify-center play-animateds">
                {isPlaying && (song.id === currSong.id) ?
                    <td className='flex equaliser-animated'>
                        <img className='equaliser-animated' width="14" height="14" alt="" src="https://open.spotifycdn.com/cdn/images/equaliser-animated-green.f5eb96f2.gif"></img>
                    </td>
                    :
                    <td className="flex song-idx">
                        {idx + 1}
                    </td>
                }
                ...
                <td onClick={onPlay} className="flex icon display-none play">
                    {isPlaying && (song.id === currSong.id) ?
                        <svg role="img" height="24" width="24" aria-hidden="true" class="Svg-sc-ytk21e-0 ldgdZj UIBT7E6ZYMcSDl1KL62g " viewBox="0 0 24 24" data-encore-id="icon"><path d="M5.7 3a.7.7 0 0 0-.7.7v16.6a.7.7 0 0 0 .7.7h2.6a.7.7 0 0 0 .7-.7V3.7a.7.7 0 0 0-.7-.7H5.7zm10 0a.7.7 0 0 0-.7.7v16.6a.7.7 0 0 0 .7.7h2.6a.7.7 0 0 0 .7-.7V3.7a.7.7 0 0 0-.7-.7h-2.6z"></path></svg>
                        :
            ...
            <td className="song-title flex">
                <div className="flex align-center ">
                    ...
                    <div className='flex column'>
                        {isPlaying && (song.id === currSong.id) ?
                            <p style={{ color: "rgb(30, 215, 96)" }}>{song.title}</p>
                            :
                            <p>{song.title}</p>
                        }
                        <span>Artist</span>
            ...
            <td className="flex">
                <div onClick={onLike} className='like icon flex'>
                    {user.stations[0].songs.find(songUser => songUser.id===song.id) ?
                        <svg role="img" height="16" width="16" aria-hidden="true" viewBox="0 0 16 16" data-encore-id="icon" class="Svg-sc-ytk21e-0 haNxPq green-like"><path d="M15.724 4.22A4.313 4.313 0 0 0 12.192.814a4.269 4.269 0 0 0-3.622 1.13.837.837 0 0 1-1.14 0 4.272 4.272 0 0 0-6.21 5.855l5.916 7.05a1.128 1.128 0 0 0 1.727 0l5.916-7.05a4.228 4.228 0 0 0 .945-3.577z"></path></svg>
                        :
                        <svg role="img" height="16" width="16" aria-hidden="true" viewBox="0 0 16 16" data-encore-id="icon" class="Svg-sc-ytk21e-0 ldgdZj hidden"><path d="M1.69 2A4.582 4.582 0 0 1 8 2.023 4.583 4.583 0 0 1 11.88.817h.002a4.618 4.618 0 0 1 3.782 3.65v.003a4.543 4.543 0 0 1-1.011 3.84L9.35 14.629a1.765 1.765 0 0 1-2.093.464 1.762 1.762 0 0 1-.605-.463L1.348 8.309A4.582 4.582 0 0 1 1.689 2zm3.158.252A3.082 3.082 0 0 0 2.49 7.337l.005.005L7.8 13.664a.264.264 0 0 0 .311.069.262.262 0 0 0 .09-.069l5.312-6.33a3.043 3.043 0 0 0 .68-2.573 3.118 3.118 0 0 0-2.551-2.463 3.079 3.079 0 0 0-2.612.816l-.007.007a1.501 1.501 0 0 1-2.045 0l-.009-.008a3.082 3.082 0 0 0-2.121-.861z"></path></svg>
                    }
                </div>
